#include "AbstractorEHS2.h"
#include <cmath>

const array<pair<double, double>, 20> AbstractorEHS2::flopBucket = {
        {
                {0.101456, 0.188832},
                {0.123186, 0.231488},
                {0.141547, 0.267828},
...
const array<pair<double, double>, 20> AbstractorEHS2::turnBucket = {
        {
                {0.0526015, 0.123803},
                {0.0749334, 0.186605},
                {0.0997095, 0.234695},
...
string AbstractorEHS2::rankToChar(int rank) {
...
string AbstractorEHS2::getFlopInfo(const PokerState &state) {
...
string AbstractorEHS2::getTurnInfo(const PokerState &state) {
    double ehs = state.pokerHand.val[PokerHand::HS][state.round - 1][state.getCurrentPlayer()];
    double ehs2 = state.pokerHand.val[PokerHand::HS2][state.round - 1][state.getCurrentPlayer()];
    for (int bucket = 0; bucket < 20; ++bucket) {
...
string AbstractorEHS2::getRiverInfo(const PokerState &state) {
    double ehs = state.pokerHand.val[PokerHand::HS][state.round - 1][state.getCurrentPlayer()];
    return to_string((int) floor((long double) ehs * 40.0));
}

string AbstractorEHS2::getWPInfo(const PokerState &state) {
    if (state.round == 1) {
...
string AbstractorEHS2::getCardInfo(const PokerState &state) {
    if (state.round == 0) {
        uint64_t cards;
...
string AbstractorEHS2::getInfoSet(PokerState state) {
